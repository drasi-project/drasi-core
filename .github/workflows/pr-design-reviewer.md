...
on:
  ...
  pull_request:
    types: [labeled]
if: github.event_name == 'workflow_dispatch' || github.event.label.name == 'review:design' || github.event.label.name == 'review:all'
permissions:
  contents: read
  pull-requests: read
network:
...
This workflow is triggered either via `workflow_dispatch` (with an explicit `pr_url` input) or via `pull_request_target` when the `review:design` or `review:all` label is added to a PR.
