...
// SAFETY: Transaction<'static, OptimisticTransactionDB> is Send but !Sync.
// We only access it through a Mutex, which provides synchronization.
// The Mutex<Option<Transaction>> is always accessed inside spawn_blocking
// (sync context), and the Mutex ensures exclusive access.
unsafe impl Sync for RocksDbSessionState {}
...
impl RocksDbSessionState {
    ...
    /// Panics if a transaction is already active (programming error).
    ...
    pub(crate) fn rollback(&self) {
        if let Ok(mut guard) = self.active_txn.lock() {
