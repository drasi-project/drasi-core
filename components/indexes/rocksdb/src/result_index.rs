        ...
        let task = task::spawn_blocking(move || {
            ...
            session_state.with_txn(|txn| {
                ...
                match &source_position_owned {
                    ...
                    None => {
                        txn.delete_cf(&metadata_cf, &sp_key)
                            .map_err(IndexError::other)?;
                    }
                }
            ...
            session_state.with_txn(|txn| {
                ...
                match seq_data {
                    Ok(Some(v)) => {
                        ...
                        // Scan for all per-source position keys (prefix "sp:")
                        let mut source_positions = std::collections::HashMap::new();
                        let prefix = b"sp:";
                        let iter = txn.prefix_iterator_cf(&metadata_cf, prefix);
                        for item in iter {
                            match item {
                                Ok((key, value)) => {
                                    if !key.starts_with(prefix) {
                    ...
                    Ok(None) => Ok(ResultCheckpoint::default()),
                    Err(e) => Err(IndexError::other(e)),
                }
            })
        });
