...
impl LazySortedSetStore for GarnetResultIndex {
    ...
    async fn get_next(
        ...
        // Load all members to merge with buffer deltas.
        // We cannot use a targeted ZRANGEBYSCORE + LIMIT 1 because buffer
        // additions/removals may shift which entry is "next". These sets are
        // per-aggregation-group and typically small.
        let mut con = self.connection.clone();
