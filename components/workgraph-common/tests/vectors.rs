...
use drasi_workgraph_common::{
    ...
    ids::{body_digest, event_id, run_id, EVENT_ID_DOMAIN, RUN_ID_DOMAIN},
...
const BODY: &str = "Please validate this issue.\n\nworkgraph:validate\n";
...
fn sample_events() -> Vec<(&'static str, WorkGraphEvent)> {
    ...
    let execution =
        ExecutionId::from_suffix("6f0a6b2f-6f1e-52b2-9f34-6d21b1f9a5c7").expect("execution id");
...
fn build_vectors() -> Value {
    ...
    let body_digests: Vec<Value> = [
        ...
        ("edited-body", Some("Please validate this issue.\n")),
    ]
    .into_iter()
    .map(|(name, body)| {
        json!({
    ...
    let run_ids: Vec<Value> = [
        ("sample", ITEM, SUBJECT, BODY),
        ("other-project-item", "PVTI_other", SUBJECT, BODY),
        ("other-subject", ITEM, "I_other", BODY),
        (
            "edited-body",
            ITEM,
            SUBJECT,
    ...
    .map(|(name, item, subject, body)| {
        ...
        json!({
            ...
            "runId": run_id(item, subject, &digest).as_str(),
        })
    })
    .collect();
...
fn committed_vectors_replay_against_the_implementation() {
    ...
    for case in committed["runIds"].as_array().expect("runIds") {
        ...
        assert_eq!(
            run_id(
                case["projectItemNodeId"].as_str().expect("item"),
                case["subjectNodeId"].as_str().expect("subject"),
                &digest
            )
            .as_str(),
