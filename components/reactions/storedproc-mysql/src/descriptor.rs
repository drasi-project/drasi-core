...
//! Descriptor for the MySQL stored procedure reaction plugin.
...
use crate::{MySqlStoredProcReaction, QueryConfig, TemplateSpec};
...
#[openapi(components(schemas(
    MySqlStoredProcReactionConfigDto,
    StoredProcQueryConfigDto,
    StoredProcTemplateSpecDto,
)))]
struct MySqlStoredProcReactionSchemas;

/// Descriptor for the MySQL stored procedure reaction plugin.
pub struct MySqlStoredProcReactionDescriptor;

#[async_trait]
impl ReactionPluginDescriptor for MySqlStoredProcReactionDescriptor {
    fn kind(&self) -> &str {
        "storedproc-mysql"
    }
    ...
    fn config_schema_json(&self) -> String {
        let api = MySqlStoredProcReactionSchemas::openapi();
    ...
    async fn create_reaction(
        ...
        let dto: MySqlStoredProcReactionConfigDto = serde_json::from_value(config_json.clone())?;
