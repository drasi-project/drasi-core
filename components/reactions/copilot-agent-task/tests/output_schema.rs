...
//! Regression test keeping `schema/workgraph-execution-state-v1.schema.json` in
//! lock-step with the Rust function it is generated from (`src/state.rs`).
//!
//! The execution-state document is the reaction's durable/operational telemetry
//! shape — it is never posted as a comment. Set `DRASI_UPDATE_SCHEMA=1` to
//! regenerate the committed file instead of asserting equality; `make
//! update-schema` wraps this.
...
fn assert_in_sync(name: &str, generated: &serde_json::Value) {
    ...
    assert_eq!(
        committed,
        rendered,
        "{} is out of sync with src/state.rs — run `make update-schema`",
        path.display()
    );
}
