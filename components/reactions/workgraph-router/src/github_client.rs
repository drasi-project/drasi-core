...
//!
//! This client exposes exactly the operations routing needs:
//!
...
//! | `list_issue_comments` | read | read the trusted assignment, start, and completion |
//! | `create_issue_comment` | write | post one `RoutingDecided` comment |
//! | `update_project_status` | write | set one single-select status option |
//!
//! The router never reads repository contents: the agent profile blob was
//! already pinned and verified when the responsibility was assigned and when
//! the execution was launched, so re-pinning it here would only let an
//! unrelated later edit to the profile file wedge a completed validation.
//!
...
const USER_AGENT_VALUE: &str = "drasi-workgraph-router";

const PROJECT_SNAPSHOT_QUERY: &str = r#"
query WorkgraphRouterProjectSnapshot($projectId: ID!, $projectItemId: ID!, $statusFieldName: String!) {
...
mutation WorkgraphRouterUpdateProjectStatus(
  $projectId: ID!
  $projectItemId: ID!
  $statusFieldId: ID!
