# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).
<!-- generated by git-cliff -->

## [0.3.5] - 2026-06-16

### Miscellaneous Tasks

- Updated the following local packages: drasi-lib, drasi-plugin-sdk, drasi-reaction-http

<!-- generated by git-cliff -->

## [0.3.4] - 2026-06-11

### Miscellaneous Tasks

- Updated the following local packages: drasi-lib, drasi-reaction-http

<!-- generated by git-cliff -->

## [0.3.3] - 2026-06-03

### Miscellaneous Tasks

- Updated the following local packages: drasi-lib, drasi-plugin-sdk, drasi-reaction-http

<!-- generated by git-cliff -->

## [0.3.2] - 2026-05-21

### Features

- Secret store plugin system with file, keyring, and Azure Key Vault providers ([#460](https://github.com/drasi-project/drasi-core/issues/460))

### Miscellaneous Tasks

- Normalize Apache-2.0 license headers across all .rs files ([#462](https://github.com/drasi-project/drasi-core/issues/462))

<!-- generated by git-cliff -->

## [0.3.1] - 2026-05-20

### Miscellaneous Tasks

- Updated the following local packages: drasi-lib, drasi-plugin-sdk, drasi-reaction-http

<!-- generated by git-cliff -->

## [0.3.0] - 2026-05-12

### Miscellaneous Tasks

- Release ([#446](https://github.com/drasi-project/drasi-core/issues/446))

<!-- generated by git-cliff -->

## [0.2.14] - 2026-05-11

<!-- generated by git-cliff -->

## [0.2.13] - 2026-04-28

### Miscellaneous Tasks

- Updated the following local packages: drasi-plugin-sdk, drasi-reaction-http

<!-- generated by git-cliff -->

## [0.2.12] - 2026-04-27

### Miscellaneous Tasks

- Updated the following local packages: drasi-plugin-sdk, drasi-lib, drasi-reaction-http

<!-- generated by git-cliff -->

## [0.2.10] - 2026-04-23

<!-- generated by git-cliff -->

## [0.2.8] - 2026-03-16

### Miscellaneous Tasks

- Updated the following local packages: drasi-lib, drasi-plugin-sdk

<!-- generated by git-cliff -->
<!-- generated by git-cliff -->
## [0.2.6] - 2026-02-26

### Miscellaneous Tasks

- Updated the following local packages: drasi-lib

<!-- generated by git-cliff -->
## [0.2.5] - 2026-02-13

### Miscellaneous Tasks

- Updated the following local packages: drasi-lib

<!-- generated by git-cliff -->
## [0.2.4] - 2026-02-11

### Miscellaneous Tasks

- Updated the following local packages: drasi-lib

<!-- generated by git-cliff -->
## [0.2.3] - 2026-02-10

### Miscellaneous Tasks

- Updated the following local packages: drasi-lib

<!-- generated by git-cliff -->
## [0.2.2] - 2026-02-05

### Miscellaneous Tasks

- Updated the following local packages: drasi-lib

<!-- generated by git-cliff -->
<!-- generated by git-cliff -->
