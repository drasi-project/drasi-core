...
mod protocol_frames {
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn binary_messages_are_skipped_before_valid_text() {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let address = listener.local_addr().unwrap();
        let server = tokio::spawn(async move {
            let (stream, _) = listener.accept().await.unwrap();
        ...
        let event = timeout(STEP_TIMEOUT, receiver.recv())
            ...
            .unwrap()
            .unwrap();
        assert_change(&event.event, "after-binary", false);
        assert_eq!(source.status().await, ComponentStatus::Running);
        source.stop().await.unwrap();
        timeout(STEP_TIMEOUT, server).await.unwrap().unwrap();
    }
    ...
    async fn retries_http_429_then_connects() {
        ...
        }) = update_rx.try_recv()
        {
            if let Some(message) = message {
                saw_retry_status |=
                    status == ComponentStatus::Starting && message.contains("retrying");
                for secret in [
                    "path-secret",
                    "query-secret",
                    "header-secret",
...
mod backpressure {
    ...
    async fn stop_interrupts_dispatch_blocked_by_subscriber_backpressure() {
        ...
        let server = tokio::spawn(async move {
            ...
            match timeout(STEP_TIMEOUT, socket.next())
                .await
                .expect("client must answer Ping while dispatch is blocked")
                .expect("client must keep the socket open")
                .expect("client Pong must be readable")
            {
                Message::Pong(payload) => assert_eq!(payload, vec![9]),
                other => panic!("expected Pong while dispatch was blocked, got {other:?}"),
            }
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn shutdown_abandons_a_saturated_frame_queue_in_delivery_order() {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let address = listener.local_addr().unwrap();
        ...
        let server = tokio::spawn(async move {
            ...
            socket.send(Message::Ping(vec![7])).await.unwrap();

            assert!(
                timeout(Duration::from_secs(1), socket.next())
                    .await
                    .is_err(),
                "a saturated frame queue must pause reads before the trailing Ping"
            );
        ...
        let source = WebSocketSource::new(
            "websocket-saturated-shutdown-test",
            WebSocketSourceConfig {
                url: format!("ws://{address}/events"),
                allow_insecure: true,
                mappings: vec![sensor_mapping()],
        ...
        let response = source
            .subscribe(SourceSubscriptionSettings {
                source_id: "websocket-saturated-shutdown-test".to_string(),
                ..subscription_settings()
            })
            .await
            .unwrap();
        ...
        source.start().await.unwrap();
        let warmup = timeout(STEP_TIMEOUT, receiver.recv())
            .await
            .unwrap()
            .unwrap();
        assert_change(&warmup.event, "warmup", false);
        start_burst_tx.send(()).unwrap();
        timeout(STEP_TIMEOUT, saturated_rx).await.unwrap().unwrap();

        timeout(Duration::from_secs(1), source.stop())
            .await
            .expect("stop must complete after the bounded queue drain")
            .unwrap();

        let first_queued = timeout(STEP_TIMEOUT, receiver.recv())
            .await
            .unwrap()
            .expect("the first burst event must already be buffered");
        assert_change(&first_queued.event, "event-0", false);
        assert!(timeout(STEP_TIMEOUT, receiver.recv())
            .await
            .unwrap()
            .is_err());
...
mod lifecycle {
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn reconnect_disabled_clean_close_stops_without_retrying() {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let address = listener.local_addr().unwrap();
        let server = tokio::spawn(async move {
            let (stream, _) = listener.accept().await.unwrap();
            ...
            match timeout(STEP_TIMEOUT, socket.next()).await {
                Ok(Some(Ok(Message::Close(_)))) | Ok(None) => {}
                other => panic!("expected client Close response, got {other:?}"),
            }
        ...
        let _response = source
            .subscribe(SourceSubscriptionSettings {
                source_id: "websocket-clean-close-test".to_string(),
                ..subscription_settings()
            })
            .await
            .unwrap();
        ...
        source.start().await.unwrap();
        wait_for_status(&source, ComponentStatus::Stopped).await;
        timeout(STEP_TIMEOUT, server).await.unwrap().unwrap();
        source.stop().await.unwrap();
    }

    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn reconnect_disabled_unclean_close_fails_without_retrying() {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let address = listener.local_addr().unwrap();
        let server = tokio::spawn(async move {
            let (stream, _) = listener.accept().await.unwrap();
        ...
        let _response = source
            .subscribe(SourceSubscriptionSettings {
                source_id: "websocket-unclean-close-test".to_string(),
                ..subscription_settings()
            })
            .await
            .unwrap();
