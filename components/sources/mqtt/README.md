...
- `relations` entries use `from`, `to`, and `label` only.
- Relation IDs are generated by the source at runtime with this rule:
  - `relation_id = <resolved_from> + "_" + <label.to_lowercase()> + "_" + <resolved_to>`
- `resolved_from` and `resolved_to` are the final endpoint values after resolving templates/placeholders.
- The generated relation ID is used in two places:
  - relation `element.id`
  - relation property `properties.id`
- `from` and `to` accept either topic-template IDs (for example, `{room}:{device}`) or payload references prefixed with `$.` (for example, `$.gateway_id`).
- Payload references in `from` or `to` are only valid when `properties.mode` is `payload_spread`.

Examples:

- `from = r1:d2`, `label = LOCATED_IN_GATEWAY`, `to = gateway_1` -> `r1:d2_located_in_gateway_gateway_1`
- `from = r1`, `label = LOCATED_IN_FLOOR`, `to = f2` -> `r1_located_in_floor_f2`
