...
- `payload_as_field`: stores the incoming payload under a single property. This mode requires `field_name`. JSON objects are rejected in this mode.
- `payload_spread`: spreads object payload fields into graph properties. This mode must not set `field_name`. Primitive values are rejected in this mode.
...
Relation mapping notes:

- Each `relations` entry has `label`, `id`, `from`, and `to`.
- `from` and `to` are objects of shape `{ label, id }`. The `id` is a template; the `label` is the graph label applied to the endpoint node.
- The `id` field on the relation, on `from`, and on `to` accepts mixed templates: topic placeholders like `{room}` and payload references like `$.gateway_id` may appear together in the same string (for example `"{device}-connect-$.gateway_id"`).
- Payload references (`$.path`) are only valid when `properties.mode` is `payload_spread`. Validation rejects them under `payload_as_field` mode.
- When an endpoint `id` resolves to a value that is not already covered by the entity or by a `nodes:` entry, the source upserts a node for that endpoint using the endpoint `label`. This makes payload-derived endpoints (for example `to: { label: Gateway, id: "$.gateway_id" }`) usable in queries without declaring them in `nodes:`.
- The relation `id` and endpoint `id` are stored on the emitted `MqttElement::Relation.id` and `MqttElement::Relation.properties.id`.
  ...
  - pattern: building/{floor}/{room}/{device}
    ...
    relations:
      - label: LOCATED_IN_ROOM
        id: "{room}:{device}_located_in_room_{room}"
        from: { label: Device, id: "{room}:{device}" }
        to: { label: Room, id: "{room}" }
      - label: LOCATED_IN_FLOOR
        id: "{room}_located_in_floor_{floor}"
        from: { label: Room, id: "{room}" }
        to: { label: Floor, id: "{floor}" }
      - label: LOCATED_IN_GATEWAY
...
- `src/descriptor.rs` — implements `SourcePluginDescriptor` with kind `"mqtt"`, configuration DTO, and OpenAPI schema generation
