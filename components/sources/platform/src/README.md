...
The Platform Source enables drasi-lib to consume events from external Drasi Platform sources via Redis Streams, allowing integration with the broader Drasi Platform infrastructure.
...
The platform source acts as a bridge between external Drasi Platform sources and drasi-lib queries, consuming events from a Redis Stream and publishing them as `SourceChange` events to internal query channels.
...
let mut core = DrasiLib::new(Arc::new(RuntimeConfig::from(config)));
