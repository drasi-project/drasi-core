# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).
<!-- generated by git-cliff -->

## [0.2.13] - 2026-09-26

### Miscellaneous Tasks

- Updated the following local packages: drasi-core, drasi-lib, drasi-plugin-sdk, drasi-index-rocksdb, drasi-postgres-common, drasi-bootstrap-postgres, drasi-reaction-application

<!-- generated by git-cliff -->

## [0.2.12] - 2026-09-24

<!-- generated by git-cliff -->

## [0.2.11] - 2026-09-17

<!-- generated by git-cliff -->

## [0.2.10] - 2026-08-28

### Miscellaneous Tasks

- Updated the following local packages: drasi-plugin-sdk, drasi-index-rocksdb, drasi-bootstrap-postgres, drasi-reaction-application

<!-- generated by git-cliff -->

## [0.2.9] - 2026-08-26

### Miscellaneous Tasks

- Updated the following local packages: drasi-lib, drasi-index-rocksdb, drasi-plugin-sdk, drasi-bootstrap-postgres, drasi-reaction-application

<!-- generated by git-cliff -->

## [0.2.8] - 2026-08-11

### Bug Fixes

- Prefer text decode for float4/float8 when pgoutput payload is printable ASCII (same heuristic as int/time)
- Omit unchanged TOAST (`b'u'`) properties instead of emitting Null so UPDATE does not clobber stored large values
- Decode pgoutput **text** for uuid/date/time/jsonb/bytea so CDC no longer drops entire rows ([#669](https://github.com/drasi-project/drasi-core/issues/669))
- Convert CDC properties via shared `PostgresValue::to_element_value` (preserve temporals; numeric→Float; include Null keys) for bootstrap parity ([#672](https://github.com/drasi-project/drasi-core/issues/672))
- Per-column decode failures fall back instead of aborting the WAL message ([#669](https://github.com/drasi-project/drasi-core/issues/669))
- **source-postgres:** Auto-detect primary keys for stable CDC element IDs ([#659](https://github.com/drasi-project/drasi-core/issues/659))

### Features

- Depend on `drasi-postgres-common` for canonical type mapping shared with bootstrap

### Miscellaneous Tasks

- End-to-end nightly build (drasi-core producer + release tooling refactor) ([#615](https://github.com/drasi-project/drasi-core/issues/615))

<!-- generated by git-cliff -->

## [0.2.7] - 2026-07-08

### Bug Fixes

- **source/postgres:** Propagate all row-changes in multi-row transactions ([#599](https://github.com/drasi-project/drasi-core/issues/599)) ([#600](https://github.com/drasi-project/drasi-core/issues/600))

<!-- generated by git-cliff -->

## [0.2.6] - 2026-06-18

### Miscellaneous Tasks

- Updated the following local packages: drasi-core, drasi-lib, drasi-plugin-sdk, drasi-index-rocksdb, drasi-bootstrap-postgres, drasi-reaction-application

<!-- generated by git-cliff -->

## [0.2.5] - 2026-06-16

### Bug Fixes

- Preserve datetime types from PostgreSQL ([#283](https://github.com/drasi-project/drasi-core/issues/283))

<!-- generated by git-cliff -->

## [0.2.4] - 2026-06-11

### Bug Fixes

- Use shared drop worker thread to prevent TLS destructor race on macOS arm64 ([#518](https://github.com/drasi-project/drasi-core/issues/518))

<!-- generated by git-cliff -->

## [0.2.3] - 2026-06-03

### Features

- **postgres:** Add replay support for checkpoint-based recovery ([#457](https://github.com/drasi-project/drasi-core/issues/457))

<!-- generated by git-cliff -->

## [0.2.2] - 2026-05-21

### Features

- Secret store plugin system with file, keyring, and Azure Key Vault providers ([#460](https://github.com/drasi-project/drasi-core/issues/460))

### Miscellaneous Tasks

- Normalize Apache-2.0 license headers across all .rs files ([#462](https://github.com/drasi-project/drasi-core/issues/462))

<!-- generated by git-cliff -->

## [0.2.1] - 2026-05-20

### Features

- **plugin-sdk:** Add SchemaUiAnnotator and x-ui annotations for plugin UI forms ([#421](https://github.com/drasi-project/drasi-core/issues/421))

<!-- generated by git-cliff -->

## [0.2.0] - 2026-05-12

### Bug Fixes

- Lossless config persistence — preserve ConfigValue envelopes, camelCase keys, and passwords ([#422](https://github.com/drasi-project/drasi-core/issues/422))

### Miscellaneous Tasks

- Release ([#446](https://github.com/drasi-project/drasi-core/issues/446))

<!-- generated by git-cliff -->

## [0.1.18] - 2026-05-11

### Bug Fixes

- Lossless config persistence — preserve ConfigValue envelopes, camelCase keys, and passwords ([#422](https://github.com/drasi-project/drasi-core/issues/422))

<!-- generated by git-cliff -->

## [0.1.17] - 2026-04-28

### Miscellaneous Tasks

- Updated the following local packages: drasi-plugin-sdk

<!-- generated by git-cliff -->

## [0.1.16] - 2026-04-27

### Miscellaneous Tasks

- Updated the following local packages: drasi-core, drasi-plugin-sdk, drasi-lib

<!-- generated by git-cliff -->

## [0.1.14] - 2026-04-23

### Bug Fixes

- Upgrade dependencies to resolve cargo-audit security vulnerabilities ([#331](https://github.com/drasi-project/drasi-core/issues/331))

<!-- generated by git-cliff -->

## [0.1.12] - 2026-03-16

### Miscellaneous Tasks

- Updated the following local packages: drasi-core, drasi-lib, drasi-plugin-sdk

<!-- generated by git-cliff -->
## [0.1.11] - 2026-03-09

### Miscellaneous Tasks

- Updated the following local packages: drasi-plugin-sdk

<!-- generated by git-cliff -->
## [0.1.10] - 2026-03-04

### Features

- Plugin API (#298)

### Fix

- Postgres bootstrap `effective_from` unit mismatch (#284)

<!-- generated by git-cliff -->
## [0.1.9] - 2026-02-26

### Miscellaneous Tasks

- Updated the following local packages: drasi-core, drasi-lib

<!-- generated by git-cliff -->
## [0.1.8] - 2026-02-13

### Miscellaneous Tasks

- Updated the following local packages: drasi-lib

<!-- generated by git-cliff -->
## [0.1.7] - 2026-02-12

### Miscellaneous Tasks

- Updated the following local packages: drasi-lib

<!-- generated by git-cliff -->
## [0.1.6] - 2026-02-11

### Miscellaneous Tasks

- Updated the following local packages: drasi-lib

<!-- generated by git-cliff -->
## [0.1.5] - 2026-02-10

### Bug Fixes

- PostgreSQL Source - query pg_replication_slots for actual LSN on existing slots (#254)

<!-- generated by git-cliff -->
## [0.1.4] - 2026-02-07

### Miscellaneous Tasks

- Updated the following local packages: drasi-core

<!-- generated by git-cliff -->
## [0.1.3] - 2026-02-05

### Miscellaneous Tasks

- Updated the following local packages: drasi-lib

<!-- generated by git-cliff -->
<!-- generated by git-cliff -->
