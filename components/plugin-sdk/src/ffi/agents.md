...
| Rich event payloads (StampedSourceEvent, BootstrapEvent, QueryResult) | **Serialized MessagePack bytes** (`ptr+len+drop_fn`) in `#[repr(C)]` envelopes — never `repr(Rust)` opaque pointers (issue #602) | `payload.rs` (DTOs + codec), `vtables.rs` (envelopes), `vtable_gen.rs` |
