...
//! Bolt type to Drasi ElementValue mapping.
...
/// Converts a Neo4j Bolt type value to a Drasi `ElementValue`.
...
/// Converts a Neo4j `BoltMap` to a Drasi `ElementPropertyMap`.
...
mod tests {
    ...
    fn test_list_property() {
        use neo4rs::{BoltList, BoltString};
        let list = BoltType::List(BoltList {
            value: vec![
                BoltType::String(BoltString {
                    value: "a".to_string(),
                }),
                BoltType::Integer(1_i64.into()),
            ],
        });
        let converted = bolt_type_to_element_value(&list).unwrap();
        match converted {
            ElementValue::List(items) => {
                assert_eq!(items.len(), 2);
                assert_eq!(items[0], ElementValue::String(Arc::from("a")));
                assert_eq!(items[1], ElementValue::Integer(1));
            }
    ...
    fn test_map_property() {
        ...
        map_value.insert(
            ...
            BoltType::Integer(99_i64.into()),
        );
        let bolt_map = BoltType::Map(BoltMap { value: map_value });
