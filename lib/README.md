...
env_logger = "0.10"  # Required for LogReaction
```

### Logging Setup

**Important**: The `LogReaction` uses Rust's standard `log` crate for output. You must initialize a logger (like `env_logger`) in your application for the reaction to produce console output:
...
Without this initialization, `LogReaction` will not produce any visible output.
...
async fn main() -> anyhow::Result<()> {
    // Initialize logging - required for LogReaction output
    env_logger::Builder::from_env(
        env_logger::Env::default().default_filter_or("info")
    ).init();
