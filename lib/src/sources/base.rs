...
impl SourceBase {
    ...
    /// timestamps stay nondecreasing in dispatch order even after clock rollback.
...
mod tests {
    ...
    fn next_monotonic_timestamp_is_nondecreasing() {
        ...
        assert_eq!(r2, t1, "equal timestamps must be preserved");

        let earlier = t1 - Duration::seconds(5);
        let r3 = SourceBase::next_monotonic_timestamp(&mut last, earlier);
        assert_eq!(r3, r2, "clock rollback must clamp to the last timestamp");
    ...
    async fn dispatch_preserves_timestamp_ties_and_queue_sequence_order() {
        for batch_dispatch in [false, true] {
            let params =
                SourceBaseParams::new("timestamp-ties").with_dispatch_mode(DispatchMode::Channel);
            let base = SourceBase::new(params).unwrap();
            let mut receiver = base.create_streaming_receiver().await.unwrap();
            ...
            let wrappers: Vec<_> = timestamps
                .into_iter()
                .map(|candidate| {
                    let mut wrapper = make_event(&base, None);
                    wrapper.timestamp = candidate;
                    wrapper
                })
                .collect();

            if batch_dispatch {
                base.dispatch_events_batch(wrappers).await.unwrap();
            } else {
                for wrapper in wrappers {
                    base.dispatch_event(wrapper).await.unwrap();
                }
            }
