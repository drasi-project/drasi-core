...
The `BootstrapProvider` trait and supporting types in this directory cross the dynamic plugin FFI boundary. Bootstrap providers are unique because they involve **cross-plugin communication**: a bootstrap plugin (Plugin A) provides data to a source plugin (Plugin B) mediated by the host.
...
The boundary is lossy today: the plugin side rebuilds the context with `BootstrapContext::new_minimal` (empty properties) and passes `None` for `SourceSubscriptionSettings` (see the bootstrap entry in `vtable_gen.rs`). Providers that need either only work in-process; the Oracle bootstrapper, which reads a source-supplied SCN from context properties, is one such case.
...
4. **Host-side proxy** — `components/host-sdk/src/proxies/bootstrap_provider.rs`
   ...
   - `build_ffi_bootstrap_sender()` — creates `FfiBootstrapSender` that bridges `std::sync::mpsc` → `tokio::sync::mpsc`
...
These types are NOT passed as opaque pointers — they are deconstructed into individual `FfiStr` arguments. If you add fields:

1. **Vtable function signature** — `components/plugin-sdk/src/ffi/vtables.rs` → `BootstrapProviderVtable.bootstrap_fn` — add the new parameter

2. **Plugin-side vtable gen** — `components/plugin-sdk/src/ffi/vtable_gen.rs` → the `bootstrap_fn` implementation that deconstructs `BootstrapRequest`

3. **Plugin-side proxy** — `components/plugin-sdk/src/ffi/bootstrap_proxy.rs` → `FfiBootstrapProviderProxy.bootstrap()` that calls the vtable with new args

4. **Host-side proxy** — `components/host-sdk/src/proxies/bootstrap_provider.rs` → `BootstrapProviderProxy.bootstrap()` that constructs `FfiStr` args
