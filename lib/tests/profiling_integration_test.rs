...
async fn test_profiling_end_to_end_flow() {
    ...
    let start_time = drasi_lib::profiling::timestamp_ns();
    ...
    query_profiling.query_receive_ns = Some(drasi_lib::profiling::timestamp_ns());

    // Small delay to ensure timestamps differ
    tokio::time::sleep(tokio::time::Duration::from_micros(10)).await;

    // Simulate query processing
    query_profiling.query_core_call_ns = Some(drasi_lib::profiling::timestamp_ns());
    tokio::time::sleep(tokio::time::Duration::from_micros(10)).await;
    query_profiling.query_core_return_ns = Some(drasi_lib::profiling::timestamp_ns());
    query_profiling.query_send_ns = Some(drasi_lib::profiling::timestamp_ns());
    ...
    reaction_profiling.reaction_receive_ns = Some(drasi_lib::profiling::timestamp_ns());
    tokio::time::sleep(tokio::time::Duration::from_micros(10)).await;
    reaction_profiling.reaction_complete_ns = Some(drasi_lib::profiling::timestamp_ns());
