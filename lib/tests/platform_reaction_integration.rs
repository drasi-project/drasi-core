...
use drasi_lib::queries::Query;
use drasi_lib::reactions::platform::{
...
struct MockQuery {
    ...
    dispatcher: Arc<drasi_lib::channels::BroadcastChangeDispatcher<QueryResult>>,
}

impl MockQuery {
    fn new(query_id: &str) -> Self {
        let dispatcher = Arc::new(drasi_lib::channels::BroadcastChangeDispatcher::<
            QueryResult,
        >::new(1000));
        Self {
            config: QueryConfig {
...
async fn create_test_server_with_query(query_id: &str) -> (Arc<DrasiServerCore>, Arc<MockQuery>) {
    ...
    let server_core = drasi_lib::server_core::DrasiServerCore::builder()
        .build()
        .await
    ...
    for _ in 0..10 {
        if let Ok(event) = tokio::time::timeout(Duration::from_millis(100), event_rx.recv()).await {
            if let Some(evt) = event {
                ...
                if evt.status == drasi_lib::channels::ComponentStatus::Running {
                    started = true;
                    break;
                }
            }
