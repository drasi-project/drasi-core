...
//! - Attaching an MQTT reaction to publish query diffs to a broker
//!
//! ## Running
//!
//! ```bash
...
async fn main() -> Result<()> {
    ...
    let default_template = QueryConfig {
        added: Some(TemplateSpec::with_extension(
            ...
            MqttExtension {
                topic: "stocks/{{query_name}}/added".to_string(),
        ...
        updated: Some(TemplateSpec::with_extension(
            ...
            MqttExtension {
                topic: "stocks/{{query_name}}/updated".to_string(),
                qos: MqttQoS::AtLeastOnce,
                retain: false,
                empty_payload: false,
                message_expiry_interval: None,
        ...
        deleted: Some(TemplateSpec::with_extension(
            ...
            MqttExtension {
                topic: "stocks/{{query_name}}/deleted".to_string(),
                qos: MqttQoS::AtLeastOnce,
                retain: false,
                empty_payload: false,
                message_expiry_interval: None,
    ...
    let mut mqtt_builder = MqttReaction::builder("mqtt-reaction")
        ...
        .with_url(mqtt_url.clone())
        .with_client_id("stock-monitor-example")
        .with_keep_alive(30)
        .with_clean_start(true)
        .with_max_inflight(100)
        .with_conn_timeout(5_000);

    if let (Ok(username), Ok(password)) = (
        std::env::var("MQTT_USERNAME"),
        std::env::var("MQTT_PASSWORD"),
    ) {
        mqtt_builder = mqtt_builder.with_identity_provider(Box::new(
            PasswordIdentityProvider::new(username, password),
        ));
    }

    let mqtt_reaction = mqtt_builder
        .build()
        .expect("Failed to build MQTT reaction");

    // =========================================================================
    // Step 4: Build DrasiLib
    // =========================================================================
    ...
    println!("├────────────────────────────────────────────┤");
    println!("│ MQTT Broker: {}│", pad_for_box(&mqtt_url, 25));
    println!("├────────────────────────────────────────────┤");
...
fn pad_for_box(value: &str, width: usize) -> String {
