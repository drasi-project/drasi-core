...
pub async fn collect_based_aggregation_test(config: &(impl QueryTestConfig + Send)) {
    let query = {
        let mut builder = QueryBuilder::new(queries::collect_based_aggregation_query());
    ...
    println!("Number of results: {} (should be 1 with collect())", result.len());
    
    assert_eq!(result.len(), 1, "With collect(), we should get exactly one result");
    ...
    match &result[0] {
        QueryPartEvaluationContext::Updating { after, .. } => {
            ...
            if let Some(VariableValue::List(order_quantities)) = after.get("order_quantities") {
                println!("Collected order quantities: {} items", order_quantities.len());
...
pub async fn simple_aggregation_test(config: &(impl QueryTestConfig + Send)) {
    let query = {
        let mut builder = QueryBuilder::new(queries::simple_product_aggregation_query());
        builder = config.config_query(builder).await;
        builder.build().await
    };
...
pub async fn collect_with_filter_test(config: &(impl QueryTestConfig + Send)) {
    let query = {
        let mut builder = QueryBuilder::new(queries::collect_with_filter_query());
        builder = config.config_query(builder).await;
        builder.build().await
    };
    ...
    assert!(!all_results.is_empty(), "Should have results for products with high ratings");
            ...
            QueryPartEvaluationContext::Adding { after } => {
                if let Some(product_id) = after.get("product_id").and_then(|v| v.as_str()) {
                    if let Some(VariableValue::List(high_ratings)) = after.get("high_ratings") {
                        println!("Product {} has {} high ratings (>= 4.0)", product_id, high_ratings.len());
    ...
    assert!(found_high_rating_products, "Should find at least one product with high ratings");
}

/// Test collecting objects
pub async fn collect_objects_test(config: &(impl QueryTestConfig + Send)) {
    let query = {
        let mut builder = QueryBuilder::new(queries::collect_objects_query());
        builder = config.config_query(builder).await;
        builder.build().await
    };
            ...
            QueryPartEvaluationContext::Adding { after } => {
                if let Some(product_id) = after.get("product_id").and_then(|v| v.as_str()) {
                    if let Some(VariableValue::List(review_details)) = after.get("review_details") {
                        println!("Product {} has {} review objects", product_id, review_details.len());
                        
                        // Verify each item is an object with rating and comment
                        for detail in review_details {
                            if let VariableValue::Object(obj) = detail {
                                assert!(obj.contains_key("rating"), "Review object should have rating");
                                assert!(obj.contains_key("comment"), "Review object should have comment");
                            } else {
    ...
    assert!(bootstrap_results.len() >= 2, "Should have results for products with reviews");
}

/// Test collect with mixed types
pub async fn collect_mixed_types_test(config: &(impl QueryTestConfig + Send)) {
    let query = {
        let mut builder = QueryBuilder::new(queries::collect_mixed_types_query());
        builder = config.config_query(builder).await;
        builder.build().await
    };
        ...
        match ctx {
            QueryPartEvaluationContext::Adding { after } => {
                if let Some(product_id) = after.get("product_id").and_then(|v| v.as_str()) {
                    ...
                    println!("Product {}: order_ids={:?}, ratings={:?}", product_id, 
                        order_ids.and_then(|v| if let VariableValue::List(l) = v { Some(l.len()) } else { None }),
                        ratings.and_then(|v| if let VariableValue::List(l) = v { Some(l.len()) } else { None })
                    );
                    
                    // Verify types
                    if let Some(VariableValue::List(oids)) = order_ids {
                        for oid in oids {
                            assert!(matches!(oid, VariableValue::String(_)), "Order IDs should be strings");
                        }
                    }
                    
                    if let Some(VariableValue::List(rs)) = ratings {
                        for r in rs {
                            assert!(matches!(r, VariableValue::Float(_)), "Ratings should be floats");
                        }
                    }
                }
            },
    ...
    assert!(bootstrap_results.len() >= 3, "Should have results for all products");
}

/// Test multiple collects in same WITH clause
pub async fn multiple_collects_test(config: &(impl QueryTestConfig + Send)) {
    let query = {
        let mut builder = QueryBuilder::new(queries::multiple_collects_query());
        builder = config.config_query(builder).await;
        builder.build().await
    };
    ...
    for ctx in &bootstrap_results {
        match ctx {
            QueryPartEvaluationContext::Adding { after } => {
                if let Some(product_id) = after.get("product_id").and_then(|v| v.as_str()) {
                    let ratings = after.get("ratings").and_then(|v| if let VariableValue::List(l) = v { Some(l.len()) } else { None }).unwrap_or(0);
                    let review_ids = after.get("review_ids").and_then(|v| if let VariableValue::List(l) = v { Some(l.len()) } else { None }).unwrap_or(0);
                    let review_count = after.get("review_count").and_then(|v| v.as_i64()).unwrap_or(0);
                    
                    println!("Product {}: {} ratings, {} review_ids, count={}", product_id, ratings, review_ids, review_count);
                    
                    // All three should match
                    assert_eq!(ratings, review_ids, "Number of ratings should match review_ids");
                    assert_eq!(ratings as i64, review_count, "Collected items should match count");
                }
            },
            _ => {}
        }
