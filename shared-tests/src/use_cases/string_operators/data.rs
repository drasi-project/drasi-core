...
pub fn get_bootstrap_data() -> Vec<SourceChange> {
    let raw = json!(
      {
        "nodes": [
          { "type": "node", "id": "p1", "labels": ["Product"], "properties": { "id": "p1", "name": "DrasiDB Enterprise", "category": "database" } },
          { "type": "node", "id": "p2", "labels": ["Product"], "properties": { "id": "p2", "name": "Quick Query Tool", "category": "tooling" } },
          { "type": "node", "id": "p3", "labels": ["Product"], "properties": { "id": "p3", "name": "Reactive Monitor", "category": "monitoring" } }
        ],
        "rels": []
      }
    );
