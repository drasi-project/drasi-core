    ...
    fn get_element_id(change: &SourceChange) -> Arc<str> {
        match change {
            SourceChange::Insert { element } | SourceChange::Update { element } => {
                element.get_metadata().reference.element_id.clone()
            }
    ...
    fn assert_aggregation_result(
        ...
        match &result[0] {
            QueryPartEvaluationContext::Aggregation {
                ...
                assert_eq!(
                    grouping_keys, &expected_grouping_keys,
                    "Grouping keys mismatch"
                );
                assert_eq!(
                    actual_default_before, &default_before,
                    "Default before mismatch"
                );
                assert_eq!(
                    actual_default_after, &default_after,
                    "Default after mismatch"
                );
    ...
    fn find_person_context<'a>(
        ...
        contexts.iter().find(|ctx| {
            if let QueryPartEvaluationContext::Aggregation { after, .. } = ctx {
                after
                    .get("Person")
                    .map_or(false, |p| p == &VariableValue::from(person_name))
            } else {
                false
            }
        })
...
pub async fn collect_movies(config: &(impl QueryTestConfig + Send)) {
    ...
    for node_change in nodes {
        ...
        let result = collect_query
            .process_source_change(node_change)
            .await
            .unwrap();
        assert!(
            result.is_empty(),
    ...
    let result1 = collect_query
        .process_source_change(alice_likes_hangover)
        .await
        .unwrap();
    ...
    let result2 = collect_query
        .process_source_change(alice_likes_superbad)
        .await
        .unwrap();
    ...
    let result3 = collect_query
        .process_source_change(alice_likes_inception)
        .await
        .unwrap();
    ...
    let result4 = collect_query
        .process_source_change(bob_likes_stepbrothers)
        .await
        .unwrap();
    ...
    let result5 = collect_query
        .process_source_change(carol_likes_inception)
        .await
        .unwrap();
    ...
    let result6 = collect_query
        .process_source_change(update_inception)
        .await
        .unwrap();
    ...
    let result7 = collect_query
        .process_source_change(delete_alice_superbad)
        .await
        .unwrap();
    ...
    let result8 = collect_query
        .process_source_change(delete_alice_hangover)
        .await
        .unwrap();
    ...
    collect_query
        .process_source_change(bob_likes_hangover)
        .await
        .unwrap();
    ...
    let result9 = collect_query
        .process_source_change(delete_hangover)
        .await
        .unwrap();
    ...
    let result10 = collect_query
        .process_source_change(update_stepbrothers)
        .await
        .unwrap();
    ...
    let result11 = collect_query
        .process_source_change(carol_likes_inception_again)
        .await
        .unwrap();
