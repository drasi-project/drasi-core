...
#[tokio::test]
async fn aggregating_query_update_to_failing_filter_emits_removing_when_group_empties() {
    // The same-group fast path of `reconcile_crossing_aggregate` must also
    // emit `Removing` when the group ends up empty after a filter eviction.
    // Otherwise an Updating event whose `after` row fails the WHERE clause
    // would leave a phantom identity-valued row behind.
    let query = build_query(
        "MATCH (a) WHERE a.Value1 < 10 RETURN a.Name as key, sum(a.Value1) as my_sum, min(a.Value1) as my_min",
    );
    ...
    let result = process_solution(
        ...
        QueryPartEvaluationContext::Updating {
            before: variablemap!["a" => node1.clone()],
            after: variablemap!["a" => node1_filtered_out.clone()],
            row_signature: IGNORED_ROW_SIGNATURE,
        },
    )
    .await;
