...
mod tests {
    use super::*;
    use crate::{
        evaluation::{context::QueryVariables, variable_value::VariableValue, ExpressionEvaluationContext, InstantQueryClock},
    ...
    async fn test_collect_basic() {
        ...
        let _ = collect.apply(&context, vec![val1.clone()], &mut accumulator).await.unwrap();
        let _ = collect.apply(&context, vec![val2.clone()], &mut accumulator).await.unwrap();
        let _ = collect.apply(&context, vec![val3.clone()], &mut accumulator).await.unwrap();

        // Snapshot should return all values
        let result = collect.snapshot(&context, vec![], &accumulator).await.unwrap();
    ...
    async fn test_collect_with_revert() {
        ...
        // Revert one value
        let _ = collect.revert(&context, vec![val1.clone()], &mut accumulator).await.unwrap();
    ...
    async fn test_collect_null_values() {
        ...
        let _ = collect.apply(&context, vec![VariableValue::Null], &mut accumulator).await.unwrap();
        let _ = collect.apply(&context, vec![VariableValue::Integer(42.into())], &mut accumulator).await.unwrap();
        let _ = collect.apply(&context, vec![VariableValue::Null], &mut accumulator).await.unwrap();
        let _ = collect.apply(&context, vec![VariableValue::String("test".into())], &mut accumulator).await.unwrap();
    ...
    async fn test_collect_duplicate_values() {
        ...
        let _ = collect.apply(&context, vec![val.clone()], &mut accumulator).await.unwrap();
        let _ = collect.apply(&context, vec![val.clone()], &mut accumulator).await.unwrap();
        let _ = collect.apply(&context, vec![val.clone()], &mut accumulator).await.unwrap();
    ...
    async fn test_collect_different_types() {
        ...
        let _ = collect.apply(&context, vec![VariableValue::Integer(42.into())], &mut accumulator).await.unwrap();
        let _ = collect.apply(&context, vec![VariableValue::Float(3.14.into())], &mut accumulator).await.unwrap();
        let _ = collect.apply(&context, vec![VariableValue::String("hello".into())], &mut accumulator).await.unwrap();
        let _ = collect.apply(&context, vec![VariableValue::Bool(true)], &mut accumulator).await.unwrap();
        ...
        if let VariableValue::List(list) = result {
            ...
            assert_eq!(list[0], VariableValue::Integer(42.into()));
            assert_eq!(list[1], VariableValue::Float(3.14.into()));
    ...
    async fn test_collect_revert_nonexistent() {
        ...
        let _ = collect.revert(&context, vec![val2.clone()], &mut accumulator).await.unwrap();

        let result = collect.snapshot(&context, vec![], &accumulator).await.unwrap();
        
        if let VariableValue::List(list) = result {
            assert_eq!(list.len(), 1, "Should not affect list if value doesn't exist");
            assert_eq!(list[0], val1);
        } else {
            panic!("Expected list result");
        }
    ...
    async fn test_collect_error_cases() {
        ...
        let result = collect.apply(&context, vec![VariableValue::Integer(1.into()), VariableValue::Integer(2.into())], &mut accumulator).await;
