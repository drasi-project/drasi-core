...
use crate::evaluation::functions::trigonometric::Degrees;
...
async fn degrees_too_few_args() {
    ...
    let result = degrees.call(&context, &func_expr, args.clone()).await;
    assert!(matches!(
        result.unwrap_err(),
        FunctionError {
            function_name: _,
...
#[tokio::test]
async fn degrees_too_many_args() {
    let degrees = Degrees {};
    let binding = QueryVariables::new();
    let context =
        ExpressionEvaluationContext::new(&binding, Arc::new(InstantQueryClock::new(0, 0)));
...
async fn degrees_float() {
    ...
    let result = degrees
        ...
        .await
        .unwrap();
    assert_eq!(result, (0.5_f64).to_degrees());
}

#[tokio::test]
async fn degrees_integer() {
    let degrees = Degrees {};
    let binding = QueryVariables::new();
    let context =
        ExpressionEvaluationContext::new(&binding, Arc::new(InstantQueryClock::new(0, 0)));
    ...
    let result = degrees
        ...
        .await
        .unwrap();
    assert_eq!(result, (1_f64).to_degrees());
}
