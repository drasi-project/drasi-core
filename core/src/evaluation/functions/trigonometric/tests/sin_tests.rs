...
use crate::evaluation::functions::trigonometric::Sin;
...
async fn sin_too_few_args() {
    ...
    let result = sin.call(&context, &func_expr, args.clone()).await;
    assert!(matches!(
        result.unwrap_err(),
        FunctionError {
            function_name: _,
...
async fn sin_null() {
    ...
    let result = sin.call(&context, &func_expr, args.clone()).await.unwrap();
...
async fn sin_float() {
    ...
    let result = sin.call(&context, &func_expr, args.clone()).await.unwrap();
    assert_eq!(result, (0.5_f64).sin());
}

#[tokio::test]
async fn sin_integer() {
    let sin = Sin {};
    let binding = QueryVariables::new();
    let context =
        ExpressionEvaluationContext::new(&binding, Arc::new(InstantQueryClock::new(0, 0)));
