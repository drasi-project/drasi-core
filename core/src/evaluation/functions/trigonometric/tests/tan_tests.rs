...
use crate::evaluation::functions::trigonometric::Tan;
...
async fn tan_too_few_args() {
    ...
    let result = tan.call(&context, &func_expr, args.clone()).await;
    assert!(matches!(
        result.unwrap_err(),
        FunctionError {
            function_name: _,
...
async fn tan_null() {
    ...
    let result = tan.call(&context, &func_expr, args.clone()).await.unwrap();
...
async fn tan_float() {
    ...
    let result = tan.call(&context, &func_expr, args.clone()).await.unwrap();
    assert_eq!(result, (0.5_f64).tan());
}

#[tokio::test]
async fn tan_integer() {
    let tan = Tan {};
    let binding = QueryVariables::new();
    let context =
        ExpressionEvaluationContext::new(&binding, Arc::new(InstantQueryClock::new(0, 0)));
