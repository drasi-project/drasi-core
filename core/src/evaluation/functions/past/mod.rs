...
use crate::{
    evaluation::{FunctionError, FunctionEvaluationError, ExpressionEvaluationContext},
...
impl ScalarFunction for GetVersionByTimestamp {
    async fn call(
        ...
        let metadata = match &args[0] {
            ...
            _ => return Err(FunctionError{function_name: expression.name.to_string(), error: FunctionEvaluationError::InvalidArgument(0)}),
        };
