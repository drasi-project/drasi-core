...
use super::materialized_query::MaterializedQuery;
use crate::models::{
...
async fn run_queries(changes: Vec<SourceChange>) -> (MaterializedQuery, MaterializedQuery) {
    ...
    let mut pending = MaterializedQuery::with_joins(PENDING_QUERY, vec![join.clone()]).await;
    let mut ready = MaterializedQuery::with_joins(READY_QUERY, vec![join]).await;
