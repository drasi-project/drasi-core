...
/// Shared incremental evaluation without transaction ownership or scheduling.
///
/// Owners serialize evaluation within the supplied indexes' transaction. Both
/// standalone queries and graph components use these same query algorithms.
...
impl QueryEvaluator {
    ...
    /// Evaluate using the owner's active transaction; no commit is performed here.
    pub async fn evaluate_changes(
        &self,
        input: Vec<SourceChange>,
    ) -> Result<Vec<QueryPartEvaluationContext>, EvaluationError> {
        let mut changes = Vec::new();
        for change in input {
            changes.extend(self.execute_source_middleware(change).await?);
        }
        self.process_changes_inner(changes).await
    }
    ...
    pub(crate) fn element_index(&self) -> &Arc<dyn ElementIndex> {
        &self.element_index
    }
...
impl Debug for QueryEvaluator {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("QueryEvaluator")
